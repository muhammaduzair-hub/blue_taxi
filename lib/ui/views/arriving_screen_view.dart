...
class ArrivingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BaseWidget<ArrivingSelectionViewModel>(
        model: ArrivingSelectionViewModel(repo: Provider.of(context), requestId: requestedId),
        builder: (context, model, child) => SafeArea(
            child: model.busy
                ...
                : Scaffold(
                    ...
                    else if (model.state == EnumToString.convertToString(Status.Completed))
                      Padding(
                        padding: const EdgeInsets.only(top: 16),
                        child: Align(
                          alignment: Alignment.topCenter,
                    ...
                    else if (model.state == EnumToString.convertToString(Status.Rate))
                      Padding(
                        padding: const EdgeInsets.only(top: 16),
                        child: Align(
                          alignment: Alignment.topCenter,
                    ...
                    if (model.state == EnumToString.convertToString(Status.Booked))
                      selectDriverSearchingSheet(model, context) //Checked
                    else if (model.state == EnumToString.convertToString(Status.Active))
                      selectArrivingBottomSheet(model) //Checked
                    else if (model.state == EnumToString.convertToString(Status.Dispatched))
                      selectArrivedBottomSheet(model, context) //Checked
                     else if (model.state == EnumToString.convertToString(Status.OnGoing))
                       selectDisbaledArrivingBottomSheet(model) //Checked
                    else if (model.state == EnumToString.convertToString(Status.Completed))
                      selectTipAndRateSheet(model) //Checked
                    else if (model.state == EnumToString.convertToString(Status.Tips))
