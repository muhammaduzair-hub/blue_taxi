...
List<SingleChildWidget> independentLocalServicesSample = [Provider.value(value: ApiSample())];
List<SingleChildWidget> dependentServicesSample = [ProxyProvider2<ApiSample, LocalApi,AuthRepositorySample>(
    update: (context, value, value2, previous) =>
        AuthRepositorySample(api: value, localApi: value2),
  )];
List<SingleChildWidget> uiConsumableProvidersSample = [StreamProvider<UserSample>(
    create: (context) =>
        Provider.of<AuthRepositorySample>(context, listen: false).user,
    initialData: UserSample.initial(),
  )];
List<SingleChildWidget> uiConsumableProviders = [StreamProvider<UserModel>(
    create: (context) =>
...
List<SingleChildWidget> dependentService = [
  ProxyProvider2<Api, LocalApi, AuthRepository>(
    update: (context, api, localApi, repo) => AuthRepository(api: api,localApi: localApi),
  ),
];